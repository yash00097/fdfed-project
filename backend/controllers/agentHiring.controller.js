...
export const submitApplication = async (req, res, next) => {
  try {
    ...
    const required = { firstName, lastName, dateOfBirth, gender, phone, address, city, state, pincode,
      aadharNumber, idProofUrl, drivingLicenseNumber, licenseExpiryDate, drivingLicenseUrl,
      yearsOfExperience, carKnowledge, preferredWorkingCity, employmentType,
      availableWorkingDays, languagesKnown, motivation, resumeUrl, addressProofUrl };

    const missing = Object.entries(required).filter(([, v]) => !v).map(([k]) => k);
    if (missing.length) return next(errorHandler(400, `Missing required fields: ${missing.join(', ')}`));

    if (!/^\d{10}$/.test(phone)) return next(errorHandler(400, 'Phone must be exactly 10 digits.'));
    if (!/^\d{6}$/.test(pincode)) return next(errorHandler(400, 'Pincode must be exactly 6 digits.'));
    if (!/^\d{12}$/.test(aadharNumber)) return next(errorHandler(400, 'Aadhar must be exactly 12 digits.'));
    ...
    const app = await AgentApplication.create({
      ...
      aadharNumber, idProofUrl,
      drivingLicenseNumber, licenseExpiryDate, drivingLicenseUrl,
      currentJob, previousJob, yearsOfExperience, hasCarSalesExperience: hasCarSalesExperience === 'true' || hasCarSalesExperience === true,
      carKnowledge, vehicleTransactionExperience,
      preferredWorkingCity, willingToTravel: willingToTravel === 'true' || willingToTravel === true,
      employmentType, availableWorkingDays,
      languagesKnown, customerHandlingExperience, salesExperience,
      referenceName, referencePhone,
      motivation,
